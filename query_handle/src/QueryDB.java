...
public class QueryDB {
	...
	public static String getresults (String startdate,String enddate, String event, String location) throws JsonProcessingException
	{
		...
		String query_final = "OPTIONAL MATCH (Node) where ("+query+") " + "with Node " + "MATCH (Event)--(Triple)--(Node) where Event.location contains '"+location+"' " + date_constraint+ " return Event.news_id,Event.dbid,Event.headline, Event.sentence, Event.location, Event.date, Node.name";
...
//				String headline = dataMap.get(event).get("Event.headline").toString();
	 ...
	 public static String update_json(String json) {
